					...
					<div class='col-lg-8 offset-lg-2'>
						<div class="form-group">
							<div class='row'>
										...
										if($menu_list){
											foreach($menu_list->result() as $row){
												...
												if($row->ID_Familia == 3) {
													...
													{
														echo "<option class='".$hoy."' selected value='".$row->ID_Menu."' >".$row->Menu."</option>";
													}
													else {
														echo "<option class='".$hoy."' value='".$row->ID_Menu."' >".$row->Menu."</option>";
													}
												}
								...
								<div class='col-lg-6'>
										<label>Segundo</label>
										<select class="form-control" required name='ID_Segundo' id='ID_Segundo'>
											<option value=''>-- Seleccione --</option>
											<?php
												if($menu_list){
													foreach($menu_list->result() as $row){
			...
			<div class="row">
				...
				<div class="col-lg-6 col-xs-12">
					<div class='text-right'>
						<?php $ID = encriptar($pedidodetalle->ID_Pedido) ?>

						<a href='<?php echo  base_url('Pedido/detalle/'.$ID); ?>' class="btn btn-warning"
							style="-webkit-appearance: button-bevel;" type="button">Ver Orden</a>
